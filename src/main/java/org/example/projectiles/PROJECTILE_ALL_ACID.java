...
public class PROJECTILE_ALL_ACID extends Projectile{
    static String staticEmote = EmoteConfig.getEmote(MethodHandles.lookup().lookupClass().getSimpleName().toLowerCase());

    public PROJECTILE_ALL_ACID(){
...
// data/scripts/projectiles/all_acid.lua
...
if ( #projectiles > 0 ) then
	for i,projectile_id in ipairs( projectiles ) do
		...
		if ( tags == nil ) or ( string.find( tags, "acid" ) == nil ) then
			...
			shoot_projectile_from_projectile( projectile_id, "data/entities/projectiles/deck/acidburst.xml", px, py, vel_x, vel_y )
