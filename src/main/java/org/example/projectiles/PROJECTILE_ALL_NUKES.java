...
public class PROJECTILE_ALL_NUKES extends Projectile{
    static String staticEmote = EmoteConfig.getEmote(MethodHandles.lookup().lookupClass().getSimpleName().toLowerCase());

    public PROJECTILE_ALL_NUKES(){
...
if ( #projectiles > 0 ) then
	for i,projectile_id in ipairs( projectiles ) do
		...
		if ( tags == nil ) or ( string.find( tags, "nuke" ) == nil ) then
			...
			shoot_projectile_from_projectile( projectile_id, "data/entities/projectiles/deck/nuke.xml", px, py, vel_x, vel_y )
